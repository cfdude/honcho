...
**Definition of "lossless" (made explicit per Gate 1 / C1):** after a move, every moved row retains all non-derived columns and content. Nothing is recreated-with-defaults. The session's `created_at`, `is_active`, `configuration`, `h_metadata`, `internal_metadata` and the children's `id`/`public_id` are always preserved, and auto-created target peers/collections are full-column copies (not identity-only). The session `id` is **preserved** on the primary (deferrable-constraints) and superuser (`DISABLE TRIGGER`) in-place paths; the no-privilege create-new-row fallback preserves all columns and content but **reassigns the session `id`** (and clears the session's transient `queue` rows). Transient `queue` work-state is never "lost" in a meaningful sense — it is deliberately not carried.
...
4. **Relocate, preserving identity** — the composite FKs target `(name, workspace_name)`, have no `ON UPDATE CASCADE`, and are NOT DEFERRABLE, so an in-place change of the session's `(name, workspace_name)` while children still reference the old pair would violate them mid-statement. Within the transaction, suspend that check while both sides are updated, then restore it:
   - **Primary (id-preserving; owner privilege):** make the affected composite FKs deferrable for the transaction (`ALTER TABLE … ALTER CONSTRAINT … DEFERRABLE`, then `SET CONSTRAINTS ALL DEFERRED`), UPDATE the session + children, then **`SET CONSTRAINTS ALL IMMEDIATE`** — which forces the now-consistent checks and drains the deferred queue — **before** restoring the constraints to NOT DEFERRABLE (restoring while FK events are still pending raises `cannot ALTER TABLE … pending trigger events`). `ALTER TABLE` is transactional, so a rollback reverts the constraint change too. `ALTER CONSTRAINT` requires only ownership of the child tables, not superuser; the constraint names are resolved from `pg_constraint` at runtime (SQLAlchemy does not name them deterministically). *(Simpler equivalent: leave the constraints `DEFERRABLE INITIALLY IMMEDIATE` permanently — a no-op for all normal operation — and skip the restore.)* *Equivalent on superuser roles:* `ALTER TABLE … DISABLE TRIGGER ALL` on `sessions` **and every child table**, re-enabled before commit — the default docker `postgres` role is superuser, so `DISABLE TRIGGER` is also available there. The mechanism is chosen by privilege detection (`rolsuper` / can-defer); both take a brief `ACCESS EXCLUSIVE` lock on the affected tables (`sessions`, `messages`, `message_embeddings`, `documents`, `session_peers`), so run during quiescence.
